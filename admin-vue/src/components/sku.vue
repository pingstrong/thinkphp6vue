  ...
  <div class="vue-sku">
    ...
    <el-card class="box-card">
      ...
      <section>
        ...
        >
          <div>
            <span v-if="!cacheSpecification[index].state">{{ item.name }}</span>
            <el-input
              size="small"
              style="width: 200px"
              v-if="cacheSpecification[index].state"
    ...
    <el-card>
      ...
      <table class="el-table" cellspacing="10" cellpadding="0">
        <thead>
          <tr>
          ...
          <tr>
            <td colspan="10" class="wh-foot">
              ...
              <template v-if="isSetListShow">
                <el-button @click="openBatch('childProductCost')" size="mini"
                  >成本价</el-button
                >
                <el-button @click="openBatch('childProductPrice')" size="mini"
                  >销售价</el-button
                >
                <el-button @click="openBatch('childProductStock')" size="mini"
                  >库存</el-button
                >
              </template>
              <template v-else>
          ...
          <tr :key="index" v-for="(item, index) in countSum(0)">
            ...
            <td>
              <el-input
                size="small"
                type="text"
                v-model.trim="childProductArray[index].childProductCost"
                placeholder="输入成本价"
              >
              </el-input>
            </td>
            <td>
              <el-input
                size="small"
                type="text"
                v-model.trim="childProductArray[index].childProductPrice"
                placeholder="输入销售价"
              >
              </el-input>
            </td>
...
export default {
  ...
  watch: {
    childProductArray() {
      // this.$emit("update:optionData", this.childProductArray);
      // this.$emit("update:specData", this.specification);
      // this.$emit("update:specItemData", this.addValues);
      // console.log("childProductArraychildProductArraychildProductArraychildProductArraychildProductArray")
    },
  },
  computed: {
    // 所有sku的id
  ...
  mounted() {},
  created() {
    //this.createData(this.specData, this.optionData)
  },
  methods: {
    // 创建数据
    createData(specData, optionData) {
      if (specData.length > 0) {
        this.childProductArray = optionData; //[{"childProductId":0,"childProductSpec":{"颜色":"黑色","尺寸":"s"},"childProductNo":"PRODUCTNO_0","childProductStock":20,"childProductPrice":0,"childProductCost":0},{"childProductId":0,"childProductSpec":{"颜色":"黑色","尺寸":"m"},"childProductNo":"PRODUCTNO_1","childProductStock":880,"childProductPrice":0,"childProductCost":660},{"childProductId":0,"childProductSpec":{"颜色":"黑色","尺寸":"xl"},"childProductNo":"PRODUCTNO_2","childProductStock":0,"childProductPrice":0,"childProductCost":0},{"childProductId":0,"childProductSpec":{"颜色":"白色","尺寸":"s"},"childProductNo":"PRODUCTNO_3","childProductStock":0,"childProductPrice":0,"childProductCost":0},{"childProductId":0,"childProductSpec":{"颜色":"白色","尺寸":"m"},"childProductNo":"PRODUCTNO_4","childProductStock":0,"childProductPrice":0,"childProductCost":0},{"childProductId":0,"childProductSpec":{"颜色":"白色","尺寸":"xl"},"childProductNo":"PRODUCTNO_5","childProductStock":0,"childProductPrice":0,"childProductCost":0},{"childProductId":0,"childProductSpec":{"颜色":"蓝色","尺寸":"s"},"childProductNo":"PRODUCTNO_6","childProductStock":0,"childProductPrice":0,"childProductCost":0},{"childProductId":0,"childProductSpec":{"颜色":"蓝色","尺寸":"m"},"childProductNo":"PRODUCTNO_7","childProductStock":0,"childProductPrice":0,"childProductCost":0},{"childProductId":0,"childProductSpec":{"颜色":"蓝色","尺寸":"xl"},"childProductNo":"PRODUCTNO_8","childProductStock":0,"childProductPrice":770,"childProductCost":0}];
        // const arr = ['颜色', '尺寸']
        ...
        for (let i = 0; i < specData.length; i++) {
          ...
          this.addSpec();
          // 数据
          this.specification[i].name = specData[i].name;
          this.addValues[i] = specData[i].value.join(" "); //arr2[i]
          // 缓存按钮键值
          this.cacheSpecification[i].state = false;
          this.cacheSpecification[i].name = specData[i].name;
          // 构建
          this.initSpecTag(i);
        }
      }
    ...
    initSpecTag(index) {
      ...
      console.log(index, this.cacheSpecification);
      ...
      this.cacheSpecification[index].state = false;
    },
    // 添加规格项目
    addSpec() {
    ...
    // 修改状态
    updateSpec(index) {
      this.cacheSpecification[index].state = true;
    ...
    addSpecTag(index) {
      let str = this.addValues[index] || "";
      console.log(index, this.cacheSpecification, this.addValues);
      if (!str.trim() || !this.cacheSpecification[index].name.trim()) {
        this.$message.error("名称不能为空，请注意修改");
        return;
      } // 判空
    ...
    changeStock(option, index, stockCopy) {
      ...
      if (option === "add") {
        ...
        console.log(spec, this.stockSpecArr);
        if (
          this.stockSpecArr.findIndex((item) => objEquals(spec, item)) === -1
        ) {
